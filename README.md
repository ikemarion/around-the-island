...
Download [ATI v0.52 for Windows](https://github.com/ikemarion/around-the-island/raw/refs/heads/main/releases/ATI-v0.52-Windows.zip), extract it, and run the included ATI executable. Protocol 16. New in v0.52: expressive Sockling animation—bouncy walking/running, flexible elbows and knees, jump stretch, airborne poses, soft landing squash, carrying, crouch/slide poses, blinks and stun wobble. Animations are cosmetic and reconstructed locally from existing player movement; no extra network packets or movement changes. Blender source and repeatable asset builder are included in the repository. The 2× Speed sneaker pickup doubles running/crouching speed for five seconds, activated with Q / right bumper. No stacking with the charged chaser boost. Charged chaser boost remains F / left bumper. Direct Internet joining requires a reachable public IP and UDP forwarding; normal Join still uses Playit. Joining automatically shares unsent ATI diagnostics with the configured host. See [NETWORK-RELIABILITY.md](NETWORK-RELIABILITY.md).
