...
func run() -> void:
	...
	p._update_chase_charge(6.0,true)
	assert(is_equal_approx(p._chase_speed_multiplier(),1.15))
	assert(is_equal_approx(main._player_state(0).chase_charge,1.0))
	p._update_chase_charge(1.0,false)
	assert(is_equal_approx(p.chase_charge,0.5))
