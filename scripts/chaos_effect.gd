...
func setup(effect_kind: StringName, source, target = null) -> void:
	...
	remaining = 6.0 if kind == &"hot_potato" else (4.0 if kind == &"decoy_double" else 5.0)
	...
	match kind:
		...
		&"magnet_mayhem":
			...
			for chair in get_tree().get_nodes_in_group("shoveable"):
				...
				if offset.length() < 6.0 and offset.length() > 1.0:
					chair.sleeping = false
					chair.apply_central_force(offset.normalized() * 65.0)
		&"hot_potato":
...
func release_magnet() -> void:
	if spent or replica:
		return
	spent = true
	for chair in get_tree().get_nodes_in_group("shoveable"):
		var away: Vector3 = chair.global_position - global_position
		if away.length() < 6.0:
			chair.sleeping = false
			chair.launch((away.normalized() + Vector3.UP * 0.45).normalized() * 45.0)
	if is_instance_valid(owner_player):
		owner_player._play_sfx("air_horn")
	queue_free()
