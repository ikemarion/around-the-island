...
Download [ATI v0.55 for Windows](https://github.com/ikemarion/around-the-island/raw/refs/heads/main/releases/ATI-v0.55-Windows.zip), extract it, and run the included ATI executable. Protocol 16. New in v0.55: the approved cartoon stun-gun collectible, with a rounded mint shell, large cyan lens, cream collar, curved coral grip and gold lightning badges. Its original one-shot behavior is unchanged. Removed the floating KITCHEN / LIVING ROOM / GARAGE labels over the four connecting doorways, leaving their frames and collision unchanged. [Approved reference and actual Godot captures](docs/art/stun-gun-concept/README.md).

The **Score Ribbons** floating leaderboard from v0.54 has rounded cream pennants with recessed, animated player-color bars, round YOU/P2/P3/P4 badges, a large mint-framed countdown and a gently animated gold leader crown. The layout centers the active players and keeps final scores visible after the timer ends. [Actual Godot captures and design notes](docs/art/leaderboard-concepts/v054/README.md). Score totals still show tenths of a second; the colored fills share a relative scale for comparing players, while the countdown measures round progress. The tabletop score lanes and existing Sockling sculpt/animations are unchanged. No movement, scoring-rule, collision or network-protocol changes. The 2× Speed sneaker pickup doubles running/crouching speed for five seconds, activated with Q / right bumper. No stacking with the charged chaser boost. Charged chaser boost remains F / left bumper. Direct Internet joining requires a reachable public IP and UDP forwarding; normal Join still uses Playit. Joining automatically shares unsent ATI diagnostics with the configured host. See [NETWORK-RELIABILITY.md](NETWORK-RELIABILITY.md).
